  ...
  <div>
    ...
    >
      <div class="flex items-center">
        ...
        <div
          v-if="$page.props.flash.success === 'Het ronde-bestand is verwerkt met de volgende meldingen: '"
          class="py-4 text-white text-sm font-medium"
        >
          {{ $page.props.flash.success }}
          <br>
          De volgende rondes zijn juist aangemaakt: {{ getSuccesCookie() }}
          <br>
          De volgende rondes zijn NIET juist aangemaakt: {{ getErrorCookie() }}
        </div>
        <div
          v-else
          class="py-4 text-white text-sm font-medium"
...
export default {
  ...
  methods: {
    getSuccesCookie() {
      const succesRounds = this.$cookies.get("succesvolGemaakt");
      this.$cookies.remove("succesvolGemaakt");
      return succesRounds;
    },
    getErrorCookie() {
      const errorRounds = this.$cookies.get("foutiefGemaakt");
      this.$cookies.remove("foutiefGemaakt");
      return errorRounds;
    },
  },
};
